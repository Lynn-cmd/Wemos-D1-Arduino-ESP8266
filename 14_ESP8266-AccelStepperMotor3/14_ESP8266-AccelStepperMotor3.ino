...
#define STAPSK "Aa123456"
 ...
 // Y slot
 const int ydirPin = 12;     // D6方向控制引脚
 const int ystepPin = 5;    // D3步进控制引脚
 // Z slot
 const int zdirPin = 13;     // D7方向控制引脚
 const int zstepPin = 4;    // D4步进控制引脚
//// A slot
...
void setup(void) {
  ...
  stepperx.setAcceleration(100.0);  // 设置电机加速度20.0  
  steppery.setMaxSpeed(maxspeed);     // 设置电机最大速度300 
  steppery.setAcceleration(100.0);  // 设置电机加速度20.0  
  stepperz.setMaxSpeed(maxspeed);     // 设置电机最大速度300 
  stepperz.setAcceleration(100.0);  // 设置电机加速度20.0  
